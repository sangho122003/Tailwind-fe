...
function BenefitItem({ icon }: { icon: IconData }) {
  return (
    <div className="space-y-2">
      ...
      <p className="text-[#1E1E1E] text-base leading-relaxed w-fit">{icon.dis}</p>
    </div>
  );
}

function DesktopView({ icons }: { icons: IconData[] }) {
  ...
  return (
    <section className="w-full py-20 px-6 md:px-[139px]">
      <div className="max-w-6xl mx-auto space-y-16">
        ...
        <DesktopView icons={icons} />
        <MobileView icons={icons} />
      </div>
    </section>
  );
}
